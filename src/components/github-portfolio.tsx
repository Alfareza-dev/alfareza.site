    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {repos.map((repo) => (
        <Link 
          ...
          className="group relative flex flex-col justify-between p-6 rounded-xl border border-white/10 bg-white/5 hover:bg-white/[0.08] transition-all duration-300 ease-in-out hover:border-brand-primary/30 overflow-hidden"
        >
          {/* Subtle gradient hover effect */}
          <div className="absolute inset-0 bg-gradient-to-br from-brand-primary/10 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          
          <div className="flex flex-col z-10">
            <div className="flex justify-between items-start mb-4">
              <h3 className="font-semibold text-lg tracking-tight group-hover:text-brand-primary transition-colors">
                {repo.name}
              </h3>
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground z-10">
            {repo.language && (
              <div className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-brand-primary/80"></span>
